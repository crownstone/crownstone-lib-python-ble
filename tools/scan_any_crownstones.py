...
if args.verbose:
    BleEventBus.subscribe(BleTopics.rawAdvertisment, printAdvertisements)
else:
    BleEventBus.subscribe(BleTopics.rawAdvertisment, printFullAdvertisements)

# this will start scanning
for i in range(20):
    # note: cannot use large values for startScanning. See doc.
    print("Scanning for any ble devices with service data..")
